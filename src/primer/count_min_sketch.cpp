...
template <typename KeyType>
static auto ComparePairs(const std::pair<KeyType, uint32_t> &lhs,
				const std::pair<KeyType, uint32_t> &rhs) -> bool {
...
auto CountMinSketch<KeyType>::TopK(uint16_t k, const std::vector<KeyType> &candidates)
  ...
  std::sort(results.begin(), results.end(), ComparePairs<KeyType>);
